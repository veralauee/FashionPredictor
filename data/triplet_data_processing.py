...
class TripletDataProcessing(Dataset):
    ...
    def __getitem__(self, idx):
        img1, label1, u1,v1 = self.getimage(idx)
        if idx%2 ==0: # consumer
            positive_idx = idx+1
            img2, label2, u2,v2 = self.getimage(positive_idx) # shop--positive
            
        else: # shop
            positive_idx = idx-1
            img2,label2, u2,v2 = self.getimage(positive_idx)

        negative_idx = np.random.randint(max(0,idx-20), min(len(self.label)-2, idx+2000))
        while (negative_idx == idx or negative_idx == positive_idx):
            negative_idx = np.random.randint(max(0,idx-20), min(len(self.label)-2, idx+2000))

        img3, label3, u3,v3 = self.getimage(negative_idx) # negative
        return img1,label1,u1,v1, img2,label2,u2,v2, img3, label3,u3,v3
